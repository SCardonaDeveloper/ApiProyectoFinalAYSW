...
using ApiBack.Repositorios.Abstracciones;        // Para implementar la interfaz IRepositorioLecturaTabla
using ApiBack.Servicios.Abstracciones;           // Para usar IProveedorConexion y obtener cadenas de conexión
            ...
            if (!string.IsNullOrWhiteSpace(camposEncriptar))
            {
                ...
                foreach (var campo in camposAEncriptar)
                {
                    if (datosFinales.ContainsKey(campo) && datosFinales[campo] != null)
                    {
                        ...
                        datosFinales[campo] = ApiBack.Servicios.Utilidades.EncriptacionBCrypt.Encriptar(valorOriginal);
                    }
                }
            }
